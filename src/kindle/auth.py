...
from kindle.activation_bytes import get_activation_bytes as get_ab
from kindle.aescipher import AESCipher, detect_file_encryption
from kindle.exceptions import FileEncryptionError, AuthFlowError, NoRefreshToken
from kindle.login import login, external_login
from kindle.register import deregister as deregister_
from kindle.register import register as register_
from kindle.utils import test_convert

if TYPE_CHECKING:
    import pathlib
    from kindle.localization import Locale

logger = logging.getLogger("kindle.auth")
...
class Authenticator(httpx.Auth):
    ...
    Attributes:
        ...
        crypter (:class:`~kindle.aescipher.AESCipher`, :obj:`None`):
        ...
        locale (:class:`~kindle.localization.Locale`, :obj:`None`):
    ...
    def from_file(cls,
        ...
        Args:
            ...
            locale: The country code of the Kindle marketplace to interact
            ...
            **kwargs: Keyword arguments are passed to the
                :class:`~kindle.aescipher.AESCipher` class. See below.
    ...
    def from_login(cls,
        ...
        Args:
            ...
            locale: The ``country_code`` or :class:`kindle.localization.Locale`
        ...
        Returns:
            An :class:`~kindle.auth.Authenticator` instance.
        """
        auth = cls()
        auth.locale = locale
        ...
        if register:
            ...
            logger.info("registered Kindle device")
    ...
    def from_login_external(cls,
        ...
        auth.re_login_external(
            serial=serial, login_url_callback=login_url_callback)

        logger.info("logged in to Kindle.")
