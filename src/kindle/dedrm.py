...
SYM_NAMES = [
    ...
    'signing_voucher', 'com.amazon.drm.EncryptedPage@1.0', 'cipher_text', 
    'cipher_iv', 'com.amazon.drm.Signature@1.0', 'data', 
    'com.amazon.drm.EnvelopeIndexTable@1.0', 'length', 'offset', 'algorithm', 
    'encoded', 'encryption_algorithm', 'hashing_algorithm', 'expires', 
    'format', 'id', 'lock_parameters', 'strategy', 'com.amazon.drm.Key@1.0',
    'com.amazon.drm.KeySet@1.0', 'com.amazon.drm.PIDv3@1.0',
    'com.amazon.drm.PlainTextPage@1.0', 'com.amazon.drm.PlainText@1.0', 
    'com.amazon.drm.PrivateKey@1.0', 'com.amazon.drm.PublicKey@1.0', 
    'com.amazon.drm.SecretKey@1.0', 'com.amazon.drm.Voucher@1.0', 
    'public_key', 'private_key', 'com.amazon.drm.KeyPair@1.0', 
    'com.amazon.drm.ProtectedData@1.0', 'doctype', 
    'com.amazon.drm.EnvelopeIndexTableOffset@1.0', 'enddoc', 'license_type', 
    'license', 'watermark', 'key', 'value', 'com.amazon.drm.License@1.0', 
    'category', 'metadata', 'categorized_metadata', 
    'com.amazon.drm.CategorizedMetadata@1.0', 
    'com.amazon.drm.VoucherEnvelope@1.0', 'mac', 'voucher',
    'com.amazon.drm.ProtectedData@2.0', 'com.amazon.drm.Envelope@2.0',
...
class DrmIonVoucher:
    ...
    def decryptvoucher(self):
        ...
        for item in self.drmkey:
            ...
            _assert(item["algorithm"] == "AES",
                    "Unknown cipher algorithm: %s" % item["algorithm"])
            _assert(item["format"] == "RAW",
                    "Unknown key format: %s" % item["format"])
    ...
    def parse(self):
        ...
        _assert(lockparams.ion_type == IonType.LIST,
    ...
    def parsevoucher(self):
        ...
        _assert(self.voucher["license"].ion_annotations[0].text == "com.amazon.drm.License@1.0",
            ...
            for item in ion_list:
                if item.ion_annotations[0].text in ["com.amazon.drm.EnvelopeMetadata@1.0", "com.amazon.drm.EnvelopeMetadata@2.0"]:
                    ...
                    if self.vouchername == "":
                        ...
                        _assert(self.vouchername == item["encryption_voucher"],
...
class KFXZipBook:
    def __init__(self, infile, auth: "kindle.Authenticator"):
        self.infile = infile
        self.dsn = auth.device_info["device_serial_number"]
        self.voucher = None
        self.decrypted = {}
            ...
            for filename in zf.namelist():
                with zf.open(filename) as fh:
                    ...
                    if len(outfile) > 0:
                        ...
                        print("Decrypting KFX DRMION {0} results in a length of Zero. Skip file.".format(filename))
