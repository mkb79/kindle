...
def build_client_id(serial: str) -> str:
    client_id = serial.encode() + b"#AGZZ1ST63LKBW"
...
def build_oauth_url(
    ...
    """Builds the url to login to Amazon as an Kindle device"""
    ...
    oauth_params = {
        ...
        "openid.assoc_handle": f"amzn_kindle_for_ios_{country_code}",
        "openid.identity": "http://specs.openid.net/auth/2.0/identifier_select",
        "pageId": "amzn_kindle_for_ios_branded",
...
def login(
    ...
    """Login to Amazon by simulating an Kindle App for iOS.
    
    Args:
        username: The Amazon email address.
        password: The Amazon password.
        country_code: The country code for the Kindle marketplace to login.
        domain: domain: The top level domain for the Kindle marketplace to login.
    ...
    Returns:
        An ``access_token`` with ``expires`` timestamp, 
        ``website_cookies`` from the authorized Client and the ``serial``.
    """
    ...
    oauth_url, serial = build_oauth_url(country_code, domain, serial)
    ...
    login_inputs["metadata1"] = "unknown"
...
def external_login(
    ...
    Returns:
        ...
        authorized Client and the ``serial``.
    """
    oauth_url, serial = build_oauth_url(country_code, domain, serial)
